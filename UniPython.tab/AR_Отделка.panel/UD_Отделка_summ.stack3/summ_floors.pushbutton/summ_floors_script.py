...
__doc__ = """Суммировать все полы и записать их в помещения"""
__author__ = '@unidraft'
__version__ = '0.0.0'
__title__ = "Суммировать полы"
...
areaFloors = [float(floor.get_Parameter(BuiltInParameter.HOST_AREA_COMPUTED).AsDouble()* 0.09290304) for floor in floors]
...
for room in rooms:
	if room.LookupParameter("UD_ARCH_Автоматический расчет").AsInteger() == 1:
		...
		if finalDic.ContainsKey(num):
			...
			room.LookupParameter("Площадь отделки пола").Set(str(round(sum(totalArea),2)).replace('.', ','))
t.Commit()
