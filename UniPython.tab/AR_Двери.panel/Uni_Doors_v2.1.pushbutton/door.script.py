...
__doc__ = 'Заполнение параметра UD_DR_Примечание и назначение эскизов'
__author__ = '@unidraft'
__version__ = '0.0.0'
__title__ = "Двери"
...
def imageReLoad(rb):
	...
	if rb=='rb12':
		imTypeNameMas=[imType.get_Parameter(BuiltInParameter.ALL_MODEL_TYPE_NAME).AsString() for imType in imTypeCol]
		[ImageType.Create(doc,LINK +'/'+imName) for imName in imNameMas if imName not in imTypeNameMas]
	else:
		pass
		[doc.Delete(imType.Id) for imType in imTypeCol if imType.get_Parameter(BuiltInParameter.ALL_MODEL_TYPE_NAME).AsString() in imNameMas]
