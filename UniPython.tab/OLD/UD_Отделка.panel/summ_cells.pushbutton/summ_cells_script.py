...
interimList = [[i.LookupParameter("ADSK_Примечание").AsString(),float(i.get_Parameter(BuiltInParameter.HOST_AREA_COMPUTED).AsValueString().Replace(',', '.')),GetFinishCeilingName(i),i] for i in ceilings]
[interimList.append([i.LookupParameter("ADSK_Примечание").AsString(),float(i.get_Parameter(BuiltInParameter.HOST_AREA_COMPUTED).AsValueString().Replace(',', '.')),GetFinishName(i),i]) for i in wallsFinish]
		...
		if dictFinal.ContainsKey(num):		
			...
			for index,item in enumerate(dictFinal[num], 1):
				...
				if dictFinal[num].Count > 1:			
					...
					finishName = finishName + str(index) + ". " + finish + " - " + str(sumArea) + " кв. м\n"
					
				else:
					sumArea = sum([i[1] for i in dictFinal[num][item]])
					finalArea = sumArea
					finishName = finish + " - " + str(sumArea) + " кв. м\n"
	#		print finishName
			room.LookupParameter("Отделка потолка").Set(finishName)		
			room.LookupParameter("Площадь отделки потолка").SetValueString(str(finalArea))
t.Commit()